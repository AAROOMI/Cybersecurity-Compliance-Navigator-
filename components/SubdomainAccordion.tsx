...
import type { Domain, Control, Subdomain } from '../types';
...
const GeneratedDocuments: React.FC<{ docs: GeneratedDocs }> = ({ docs }) => {
  ...
  const renderMarkdown = (markdown: string) => {
    ...
    for (const line of lines) {
      ...
      if (line.startsWith('### ')) {
        closeLists();
        html += `<h3 class="text-md font-semibold text-gray-800 mt-4 mb-2">${processedLine.substring(4)}</h3>`;
      } else if (line.startsWith('## ')) {
        closeLists();
        html += `<h2 class="text-lg font-semibold text-gray-800 mt-5 mb-2">${processedLine.substring(3)}</h2>`;
      } else if (line.startsWith('# ')) {
        closeLists();
        html += `<h1 class="text-xl font-bold text-gray-900 mt-6 mb-3">${processedLine.substring(2)}</h1>`;
      } else if (line.startsWith('- ') || line.startsWith('* ')) {
        if (inOrderedList) closeLists();
        if (!inUnorderedList) {
          html += '<ul class="list-disc list-inside space-y-1 my-2 text-gray-700">';
        ...
        if (!inOrderedList) {
          html += '<ol class="list-decimal list-inside space-y-1 my-2 text-gray-700">';
          inOrderedList = true;
        }
  ...
  const renderContent = (content: string) => {
    return <div className="text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: renderMarkdown(content) }} />;
  };
  ...
  return (
    <div className="mt-6 border border-gray-200 rounded-lg bg-white/50">
...
const GeneratedDocumentsSkeleton: React.FC = () => {
  return (
    <div className="mt-6 border border-gray-200 rounded-lg bg-white/50 overflow-hidden animate-pulse">
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-6 px-4" aria-label="Tabs">
...
const ControlDetail = React.forwardRef<HTMLDivElement, ControlDetailProps>(
  ({ control, isActive, domain, subdomain }, ref) => {
    ...
    const CopyButton: React.FC<{ textToCopy: string; sectionId: string }> = ({ textToCopy, sectionId }) => {
      ...
      return (
        <button
          ...
          className="flex items-center text-xs text-gray-500 hover:text-teal-600 transition-colors disabled:opacity-50"
    ...
    const handleGenerateDocs = async () => {
      ...
      try {
        ...
        setGeneratedDocs(parsedResponse);
    ...
    <div
      ref={ref}
      className={`bg-white rounded-lg p-6 border transition-all duration-700 ease-in-out ${
        isActive ? 'border-teal-500 ring-4 ring-teal-200 shadow-lg' : 'border-gray-200'
      }`}
      aria-live="polite"
    >
      <div className="flex items-baseline justify-between mb-4">
        <h4 className="text-md font-semibold text-teal-800 font-mono">{control.id}</h4>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
            <div className="flex justify-between items-center">
            <h5 className="text-sm font-semibold text-gray-600">Implementation Guidelines</h5>
        ...
        <div className="space-y-2">
            <div className="flex justify-between items-center">
            <h5 className="text-sm font-semibold text-gray-600">Expected Deliverables</h5>
      ...
      {control.history && control.history.length > 0 && (
          <div className="mt-8 pt-6 border-t border-gray-200">
            <button
              ...
              className="w-full flex justify-between items-center text-left text-sm font-semibold text-gray-600 hover:text-gray-800"
            ...
            {isHistoryVisible && (
              <div className="mt-4 pl-4 border-l-2 border-gray-200 space-y-6">
                {/* Current Version */}
                <div className="relative">
                  <div className="absolute top-1 -left-[26px] h-3 w-3 bg-teal-500 rounded-full border-4 border-white"></div>
                  <div className="pl-4">
                    <div className="flex items-baseline justify-between">
                        <h6 className="font-semibold text-gray-800">
                            Version {control.version}
                            <span className="ml-2 px-2 py-0.5 bg-teal-100 text-teal-800 text-xs font-semibold rounded-full">
                                Current
                            </span>
                        </h6>
                        <p className="text-sm text-gray-500">{control.lastUpdated}</p>
                    </div>
                    <p className="text-sm text-gray-600 italic mt-1">Latest approved version.</p>
                  </div>
                </div>
                ...
                {[...control.history].reverse().map((entry) => (
                  <div key={entry.version} className="relative">
                    <div className="absolute top-1 -left-[26px] h-3 w-3 bg-gray-300 rounded-full border-4 border-white"></div>
                    <div className="pl-4">
                        <div className="flex items-baseline justify-between">
                            <h6 className="font-semibold text-gray-700">Version {entry.version}</h6>
      ...
      <div className="mt-8 pt-6 border-t border-gray-200">
          <div className="flex flex-col items-start">
              <h5 className="text-md font-semibold text-gray-700 mb-2">Automation</h5>
          ...
          {error && <div className="mt-4 p-4 bg-red-100 text-red-800 border border-red-200 rounded-md text-sm">{error}</div>}
          {generatedDocs && !isGenerating && <GeneratedDocuments docs={generatedDocs} />}
      </div>
    </div>
    );
  }
...
export const SubdomainAccordion: React.FC<SubdomainAccordionProps> = ({ domain, subdomain, activeControlId, setActiveControlId }) => {
  ...
  useEffect(() => {
    // This effect manages scrolling to and temporarily highlighting a control.
    ...
    // This cleanup function runs when dependencies change or the component unmounts,
    // ensuring that pending scrolls or highlight removals are cancelled to prevent race conditions.
    return () => {
  ...
  return (
    <div className="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
      <button
        ...
        className="w-full flex justify-between items-center p-5 text-left transition-colors duration-200 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2"
        aria-expanded={isOpen}
        aria-controls={`subdomain-content-${subdomain.id}`}
      >
        <div className="flex items-center flex-1 min-w-0">
            <span className="font-mono text-teal-600 bg-teal-50 rounded px-2 py-1 text-sm font-semibold mr-4">{subdomain.id}</span>
            <div className="flex items-baseline flex-1 min-w-0">
              <span className="font-semibold text-lg text-gray-800 truncate" title={subdomain.title}>{subdomain.title}</span>
              <span className="ml-3 text-sm font-medium text-gray-500 bg-gray-100 px-2.5 py-1 rounded-full whitespace-nowrap">
      ...
      >
        <div className="p-5 border-t border-gray-200 bg-gray-50">
            <p className="text-gray-600 italic mb-6">{subdomain.objective}</p>
            <div className="space-y-4">
