...
import type { TrainingCourse, Lesson, Challenge, CategorizationItem, OrderingItem, Hotspot, UserTrainingProgress } from '../types';
import { CloseIcon, CheckCircleIcon, InformationCircleIcon } from './Icons';

// --- Challenge Components (defined within this file) ---

const MultipleChoiceChallenge: React.FC<{ challenge: Extract<Challenge, { type: 'multiple-choice' }>; onComplete: (isCorrect: boolean) => void; }> = ({ challenge, onComplete }) => {
    ...
    const getOptionClasses = (index: number) => {
        ...
        if (index === challenge.correctAnswer) return "border-green-500 bg-green-50 dark:bg-green-900/50";
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <p className="font-semibold text-gray-800 dark:text-gray-200">{challenge.question}</p>
...
export const LessonPlayer: React.FC<LessonPlayerProps> = ({ course, lesson, onClose, onCompleteLesson }) => {
    ...
    const challenges = lesson.challenge?.items || [];
    const currentChallenge = challenges[currentChallengeIndex];
    
    const handleChallengeComplete = (isCorrect: boolean) => {
        const newResults = [...challengeResults, isCorrect];
        ...
        setTimeout(() => {
            if (currentChallengeIndex < challenges.length - 1) {
                ...
                setShowSummary(true);
                const score = (newResults.filter(Boolean).length / challenges.length) * 100;
    ...
    const renderChallenge = () => {
        ...
        switch (currentChallenge.type) {
            case 'multiple-choice':
                return <MultipleChoiceChallenge challenge={currentChallenge} onComplete={handleChallengeComplete} />;
    ...
    const finalScore = useMemo(() => {
        ...
        return (challengeResults.filter(Boolean).length / challenges.length) * 100;
    }, [showSummary, challengeResults, challenges.length]);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-[100] flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <main className="flex-1 overflow-y-auto p-6 space-y-8">
                    ...
                    {challenges.length > 0 && !showSummary && challenges.length === 0 && (
                    ...
                    {challenges.length > 0 && (
                        <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
                            <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">{lesson.challenge?.title} ({currentChallengeIndex + 1}/{challenges.length})</h3>
                            ...
                            {showSummary ? (
                                <div className={`text-center p-6 rounded-lg ${finalScore >= 50 ? 'bg-green-50 dark:bg-green-900/50' : 'bg-red-50 dark:bg-red-900/50'}`}>
                                    ...
                                    <p className="text-lg mt-2">You scored {challengeResults.filter(Boolean).length} out of {challenges.length} ({finalScore.toFixed(0)}%).</p>
